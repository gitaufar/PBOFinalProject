 ...
 * @author ammar
 */
public class PokemonTanah extends Pokemon{
    
    public PokemonTanah(String nama,int damage,int hp){
        super(nama,damage,hp);
        super.setTipe("Tanah");
    }
    
    public PokemonTanah(String nama,int level){
        super(nama,level);
        super.setTipe("Tanah");
    }
    ...
    public void elementalAttack(Pokemon musuh) throws InterruptedException{
        System.out.print("Menggetarkan tanah");
        for(int i = 0;i < 3; i++){
            System.out.print(".");
            Thread.sleep(750);
        }
        if(musuh instanceof PokemonAir){
    ...
    public void printDetail(){
        System.out.printf("Nama: %s\t| Elemen: Tanah\nLevel: %d\t| Ep: %d/%d\nHp: %d\n",getNama(),getLevel(),getEp(),getEpMax(),getHp());
    }
