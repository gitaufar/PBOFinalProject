 ...
 * @author ammar
 */
public class PokemonEs extends Pokemon {

    public PokemonEs(String nama, int damage, int hp) {
        super(nama, damage, hp);
        super.setTipe("Es");
    }

    public PokemonEs(String nama, int level) {
        super(nama, level);
        super.setTipe("Es");
    }
    ...
    public void elementalAttack(Pokemon musuh) throws InterruptedException {
        System.out.print("Membekukan musuh");
        musuh.setStun(true);
        for (int i = 0; i < 3; i++) {
            System.out.print(".");
            Thread.sleep(750);
        }
        System.out.println("Musuh terkena stun 1 turn");
        if (musuh instanceof PokemonAngin) {
